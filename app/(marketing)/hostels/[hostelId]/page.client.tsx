...
type HostelDetailsClientProps = {
  initialHostelId?: string
}

export default function HostelDetailsClient({ initialHostelId }: HostelDetailsClientProps) {
  const params = useParams<{ hostelId?: string }>()
  const routeHostelId = Array.isArray(params?.hostelId) ? params.hostelId[0] : params?.hostelId
  const hostelId = initialHostelId ?? routeHostelId
